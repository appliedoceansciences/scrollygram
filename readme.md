...
- `scrollygram.py`: Ingests packets containing acoustic samples, either on `stdin` (such as when piped over ssh to a dev laptop from a remote system via a high-bandwidth link) or directly from the shared-memory ringbuffer segment written to by `cobs_to_shm` if running locally, and plots a live-scrolling linear-frequency spectrogram of each channel.
...
- `scope.py`: Similar to `scrollygram.py`, but shows an oscilloscope-like line plot representation of the raw timeseries.
